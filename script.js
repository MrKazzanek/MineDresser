...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btnOpenChangeSkin').addEventListener('click', () => {
        pendingChangeSkinUrl = state.baseSkinUrl;
        setRadioValue('changeModelType', state.modelType);
        const confirmBtn = document.getElementById('btnConfirmChangeSkin');
        if (confirmBtn) confirmBtn.disabled = false;

        const errDisplay = document.getElementById('changeSkinError');
        if (errDisplay) {
            errDisplay.style.color = "var(--text-muted)";
            errDisplay.innerText = "Current skin loaded. Choose model or select a new skin below.";
        }
