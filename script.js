...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('changeSkinModal').addEventListener('click', (e) => {
        if (e.target === document.getElementById('changeSkinModal')) {
            document.getElementById('changeSkinModal').classList.add('hidden');
        }
    });
...
function renderPresetSkins(containerId, isChangeModal) {
    ...
    defaultSkins.forEach(skin => {
        ...
        card.innerHTML = `
            ...
            <div class="preset-model-tag">${skin.model === 'alex' ? 'Slim' : 'Classic'}</div>
        `;
        card.addEventListener('click', () => {
            if (isChangeModal) {
                applyNewBaseSkin(skin.textureUrl, skin.model);
            } else {
                startGame(skin.textureUrl, skin.model);
            }
        });
...
async function fetchSkinFromUsername() {
    ...
    try {
        ...
        startGame(result.skinUrl, result.modelType);
    } catch(err) {
...
async function fetchSkinFromChangeModal() {
    ...
    try {
        const result = await fetchSkinData(username, (msg) => { errorDisplay.innerText = msg; });
        errorDisplay.innerText = "";
        applyNewBaseSkin(result.skinUrl, result.modelType);
    } catch(err) {
        errorDisplay.style.color = "var(--danger-color)";
        errorDisplay.innerText = err.message;
    }
...
function loadSkinFromFile(e) {
    ...
    reader.onload = async (event) => {
        ...
        startGame(result, detectedModel);
    };
...
function loadChangeSkinFromFile(e) {
    ...
    reader.onload = async (event) => {
        ...
        applyNewBaseSkin(result, detectedModel);
    };
    reader.readAsDataURL(file);
}
...
function applyNewBaseSkin(skinUrl, modelType) {
    ...
    if (mainViewer) {
        mainViewer.model = state.modelType === 'alex' ? 'slim' : 'default';
    }
