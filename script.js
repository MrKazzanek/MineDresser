...
async function fetchSkinFromUsername() {
    ...
    // 1. Get player UUID (CORS friendly)
    let uuid = null;
    try {
    ...
    // 2. Fetch 100% LIVE texture payload from Mojang Session Server
    // Try Netlify proxy endpoint first (zero CORS error on netlify.app), then CORS proxies
    ...
    // 3. Fallback to Minotar direct image load if proxies failed
    try {
