...
async function fetchSkinFromUsername() {
    ...
    // 1. Get player UUID via CORS-friendly PlayerDB API, then fetch LIVE texture payload from Mojang Official Session Server
    try {
        const playerDbRes = await fetch(`https://playerdb.co/api/player/minecraft/${username}`);
        if (playerDbRes.ok) {
            const playerData = await playerDbRes.json();
            const uuid = playerData.data?.player?.raw_id;

            if (uuid) {
                // Query official Mojang session server for 100% up-to-date real-time texture
                try {
                    ...
                    console.warn("Mojang session server fetch failed, using PlayerDB fallback", sessionErr);
                }

                // Fallback to PlayerDB texture if Mojang session server fails
                if (playerData.data?.player?.skin_texture) {
                    let skinUrl = playerData.data.player.skin_texture.replace(/^http:\/\//, 'https://');
        ...
        console.warn("PlayerDB fetch failed, trying Minotar fallback...", playerDbErr);
    }

    // 2. Fallback to Minotar direct image load
    try {
